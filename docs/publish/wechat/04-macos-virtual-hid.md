...
想继续深入的话：往"改写"方向走，把 listen 的 `TapAction::Pass` 换成改写逻辑，就是一个小 Karabiner（按键映射器）的骨架；往"真硬件级"方向走，Windows 侧的 `vdev-hid-win` 提供了对照——VHF（Virtual HID Framework）内核虚拟 HID 驱动，从 HID 类驱动层把虚拟设备注册成系统真件，Win10/11 通用，系列另有专文。仓库里各路线的选型笔记见 `docs/dev/macos-route-ideas.md`，全套文章索引见 vdev 仓库。
