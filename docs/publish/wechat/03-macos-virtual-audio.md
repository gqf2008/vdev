...
**把虚拟声卡跑进 coreaudiod：AudioServerPlugIn 是 C vtable，vdev 用 100% Rust 把它填满——最难的不是出声，是让时钟骗过系统。**

> 本文是 vdev 虚拟设备驱动开发系列之一（共 9 篇）。完整源码与代码位置标注见仓库对应文章。

## 一、虚拟声卡是干什么的

![配图](../images/macos-audio-01-core.png)
...
**4. 手工铺 vtable。** `AudioServerPlugInDriverInterface` 的布局必须与 C 头文件逐字段一致：`IUNKNOWN_C_GUTS`（`_reserved` + `QueryInterface` + `AddRef` + `Release`）后跟 19 个插件方法。Rust 侧用 `#[repr(C)]` 结构体 + `Option<unsafe extern "C" fn ...>` 逐项镜像， 的 `static mut VTABLE` 把每个槽位填上实现。两个 ABI 细节值得单独记住：
...
仓库与完整代码：vdev 仓库（`crates/vdev-audio`，约 2700 行 Rust 含驱动、CLI 与测试）。系列其余八篇见 `docs/community/`。
