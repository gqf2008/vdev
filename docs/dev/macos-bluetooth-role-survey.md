...
macOS 的蓝牙栈只实现了主机侧：`bluetoothd` 里所有 `sink` 语义都指**对端设备**
（`Ignoring sink's SBC Maximum Bitpool Value`、`LastA2DPSinkSupportedFeatures`…），
并且能直接看到字面量 `Advertising HFP AG`。所以"当音箱"从一开始就没有实现可依附。
...
- **音频面拿不到**：通话接通的 18.9 秒里每 3 秒重试一次 `transferAudioToComputer`
  （共 6 次），**每一次都返回 `-536870201`（`0xE00002C7 = kIOReturnUnsupported`）**；
...
**只有音频桥没了**——这与实测的**即时**（0.003 秒）`kIOReturnUnsupported` 完全吻合：
...
- ✅ **用户态确实能跟控制器说上话，且不需要 root**：
  `-[IOBluetoothHostController BluetoothHCIReadVoiceSetting:]` 返回 `kIOReturnSuccess`。
  这条事实对以后任何"绕开 IOBluetooth 类"的尝试都有用。
...
1. **先把"通话中 SCO 是否 status=0"跑出来再决定投入**。只看到 SLC 建立成功、
   ...
   是本轮实际发生过的误判。
2. **实验带阳性对照**：读类的命令（如 Read Voice Setting）先跑通，否则后面的失败
   无法区分"命令被拒"与"压根没发出去"。
