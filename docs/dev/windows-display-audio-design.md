...
> 状态：虚拟显示器（IddCx UMDF）与虚拟声卡（PortCls/WaveRT WDM）代码均已合入 main、构建与门禁通过，**并已于 2026-09-14 在 Win10 19045 x64 真机验证通过**（显示器多出第二块屏、声卡端点可开流且环回实测通过）。本文保留当时的设计与决策过程。用户已拍板：接受开测试签名 + 装 WDK，走内核驱动路线；
