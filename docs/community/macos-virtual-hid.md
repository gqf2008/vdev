...
| 路线 | 所在层 | 门槛 | 现状 |
|---|---|---|---|
| **CGEventPost / CGEventTap**（Quartz Event Services） | 纯用户态 | 公开 API，无签名要求；监听需 TCC 授权 | **现役推荐，vdev 选它** |
| IOHID 私有 API | 用户态（HID event system 深层） | 无公开头文件，随系统版本变动 | 能用，但不可依赖 |
| kext / DriverKit 虚拟 HID | 内核 / 系统扩展 | kext 在 Apple Silicon 上已死；DriverKit（dext）只支持 C++ | Karabiner-Elements 走的这条 |
...
补充几个 README 没列全、代码里有的子命令（均来自 `crates/vdev-host/src/main.rs:34-64` 的 `HidCmd` 定义）：
