...
| `vdev-mic-agent` | 双平台 | ✅ 可用 | 端侧 RNNoise 降噪注入虚拟麦克风。Windows 侧 2026-09-14 live 实测（Win10 19045）：数字探针 p50 **10.16 ms**、降噪 20 s 实时（帧时 p50 26.8 µs、CPU 0.156 % 单核）、直通→降噪 A/B **−5.8 → −34.1 dBFS**（A/B 为 2026-09-15 标度修复前的口径，需重测）；声学探针仍需真实麦克风与房间 |
