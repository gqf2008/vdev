...
fn addr(selector: u32, scope: u32) -> AudioObjectPropertyAddress {
    AudioObjectPropertyAddress { m_selector: selector, m_scope: scope, m_element: ELEM_MAIN }
}
...
fn get_cf_prop(obj: AudioObjectID, a: &AudioObjectPropertyAddress) -> Option<String> {
    ...
    let rc = unsafe {
        AudioObjectGetPropertyData(obj, a, 0, std::ptr::null(), &mut size, &mut cf as *mut CFStringRef as *mut c_void)
    };
    if rc != 0 {
        return None;
    }
...
fn get_array<T: Copy>(obj: AudioObjectID, a: &AudioObjectPropertyAddress) -> Vec<T> {
    ...
    let rc = unsafe {
        AudioObjectGetPropertyData(obj, a, 0, std::ptr::null(), &mut size, v.as_mut_ptr() as *mut c_void)
    };
    if rc != 0 {
        return Vec::new();
    }
...
fn find_device(needle: &str) -> Option<AudioDeviceID> {
    ...
    all_devices().into_iter().find(|&d| {
        device_name(d).to_lowercase().contains(&lower) || device_uid(d).to_lowercase().contains(&lower)
    })
}
...
fn stream_channels(dev: AudioDeviceID, scope: u32) -> u32 {
    ...
    for s in streams {
        if let Some(asbd) = get_prop::<AudioStreamBasicDescription>(s, &addr(STREAM_VIRTUAL_FORMAT, SCOPE_GLOBAL)) {
...
fn describe(dev: AudioDeviceID) -> DeviceInfo {
    ...
    let safety_offset_frames: u32 = get_prop(dev, &addr(DEV_SAFETY_OFFSET, SCOPE_GLOBAL)).unwrap_or(0);
    let frames_ms = |f: u32| if sr > 0.0 { f as f64 / sr * 1000.0 } else { 0.0 };
    DeviceInfo {
        id: dev,
        ...
        reported_latency_ms: frames_ms(latency_frames) + frames_ms(safety_offset_frames) + frames_ms(buffer_frames),
    }
}
...
impl Core {
    ...
    fn on_frame(&mut self, input: &[f32]) {
        ...
        // 3) blend
        let w = if self.adaptive { self.mixer.update(input, vad) } else { self.mix as f64 };
        if w >= 1.0 {
            ...
            for k in 0..FRAME {
                self.frame[k] = (w * wet_frame[k] as f64 + (1.0 - w) * self.dry_frame[k] as f64) as f32;
            }
        }
...
unsafe extern "C" fn capture_input_cb(
    ...
    let ctx = &mut *(in_ref_con as *mut Core);
    let abl = &*io_data;
    ...
    let mono = std::slice::from_raw_parts(buf.m_data as *const f32, n);

    // Split the borrows: the assembler owns the buffer, `Core` owns the model.
    let Core { rec_in, .. } = &mut *ctx;
    if rec_in.len() + n <= RECORD_SAMPLES {
        rec_in.extend_from_slice(mono);
    }

    // Safety: single audio thread, and `emit` is called synchronously.
    let asm = &mut *ASM.get();
    ...
    if !out_output_data.is_null() {
        ...
        if abl.m_number_buffers > 0 {
            ...
            if !buf.m_data.is_null() && frames > 0 {
                ...
                let dst = std::slice::from_raw_parts_mut(buf.m_data as *mut f32, frames * nch);
    ...
    if ctx.probe && !in_input_data.is_null() {
        ...
        if abl.m_number_buffers > 0 {
            ...
            if !buf.m_data.is_null() && frames > 0 {
                ...
                let src = std::slice::from_raw_parts(buf.m_data as *const f32, frames * nch);
                for f in 0..frames {
                ...
                if ctx.search.len() >= MARKER_LEN {
                    let mut prefix = Vec::new(); // small, bounded, and only on a hit path
                    if let Some(d) = detect_with(&ctx.search, &ctx.marker, ctx.threshold, &mut prefix) {
...
unsafe extern "C" fn speaker_render_cb(
    ...
    let ctx = &mut *(in_ref_con as *mut SpeakerCtx);
    let abl = &mut *io_data;
    ...
    for b in 0..abl.m_number_buffers as usize {
        let buf = &mut *abl.m_buffers.as_mut_ptr().add(b);
    ...
    for f in 0..frames {
        let v = if ctx.pos < ctx.marker.len() {
            let v = ctx.marker[ctx.pos];
            ctx.pos += 1;
            v
        } else {
            0.0
        };
        if ctx.until_next == 0 && ctx.pos == 0 {
            // just started a fresh marker: time it
            if let Ok(mut q) = ctx.pending.try_lock() {
                q.push_back(Instant::now());
            }
        }
        for b in 0..abl.m_number_buffers as usize {
            ...
            let bch = if buf.m_number_channels == 0 { nch } else { buf.m_number_channels as usize };
            let dst = std::slice::from_raw_parts_mut(buf.m_data as *mut f32, frames * bch);
            let base = f * bch;
            for c in 0..bch {
                dst[base + c] = v;
            }
        }
        ctx.until_next = ctx.until_next.saturating_sub(1);
        if ctx.pos >= ctx.marker.len() {
            ctx.pos = 0;
            ctx.until_next = ctx.period_frames;
        }
    }
    0
}
...
fn asbd(rate: f64, channels: u32, non_interleaved: bool) -> AudioStreamBasicDescription {
    ...
    AudioStreamBasicDescription {
        ...
        m_format_flags: FLAG_IS_FLOAT
            | FLAG_IS_PACKED
            | if non_interleaved { FLAG_IS_NON_INTERLEAVED } else { 0 },
...
fn new_hal_unit() -> Result<AudioUnit> {
    ...
    if rc != 0 || unit.is_null() {
        bail!("AudioComponentInstanceNew failed (OSStatus {rc})");
    }
    Ok(unit)
}

fn au_set<T>(unit: AudioUnit, prop: u32, scope: u32, element: u32, v: &T, what: &str) -> Result<()> {
    let rc = unsafe {
        AudioUnitSetProperty(unit, prop, scope, element, v as *const T as *const c_void, std::mem::size_of::<T>() as u32)
    };
    if rc != 0 {
        bail!("AudioUnitSetProperty({what}) failed (OSStatus {rc})");
    }
    Ok(())
}
...
fn build_capture_unit(device: AudioDeviceID, core: *mut Core) -> Result<AudioUnit> {
    ...
    au_set(unit, AU_PROP_ENABLE_IO, AU_SCOPE_INPUT, 1, &one, "EnableIO(input)")?;
    au_set(unit, AU_PROP_ENABLE_IO, AU_SCOPE_OUTPUT, 0, &zero, "EnableIO(output)")?;
    au_set(unit, AU_PROP_CURRENT_DEVICE, AU_SCOPE_GLOBAL, 0, &device, "CurrentDevice")?;
    ...
    au_set(unit, AU_PROP_STREAM_FORMAT, AU_SCOPE_OUTPUT, 1, &fmt, "StreamFormat(input client)")?;
    ...
    au_set(unit, AU_PROP_MAX_FRAMES_PER_SLICE, AU_SCOPE_GLOBAL, 0, &max_frames, "MaximumFramesPerSlice")?;

    let cb = AURenderCallbackStruct {
        input_proc: Some(capture_input_cb),
        input_proc_ref_con: core as *mut c_void,
    };
    au_set(unit, AU_PROP_SET_INPUT_CALLBACK, AU_SCOPE_GLOBAL, 0, &cb, "SetInputCallback")?;

    let rc = unsafe { AudioUnitInitialize(unit) };
    if rc != 0 {
        bail!("AudioUnitInitialize(capture) failed (OSStatus {rc})");
    }
    Ok(unit)
}

/// Speaker AU (acoustic probe only): play the marker on the *physical* output.
fn build_speaker_unit(device: AudioDeviceID, channels: u32, ctx: *mut SpeakerCtx) -> Result<AudioUnit> {
    let unit = new_hal_unit()?;
    let one: u32 = 1;
    let zero: u32 = 0;
    au_set(unit, AU_PROP_ENABLE_IO, AU_SCOPE_OUTPUT, 0, &one, "EnableIO(output)")?;
    au_set(unit, AU_PROP_ENABLE_IO, AU_SCOPE_INPUT, 1, &zero, "EnableIO(input)")?;
    au_set(unit, AU_PROP_CURRENT_DEVICE, AU_SCOPE_GLOBAL, 0, &device, "CurrentDevice")?;
    ...
    au_set(unit, AU_PROP_STREAM_FORMAT, AU_SCOPE_INPUT, 0, &fmt, "StreamFormat(output client)")?;

    let cb = AURenderCallbackStruct {
        input_proc: Some(speaker_render_cb),
        input_proc_ref_con: ctx as *mut c_void,
    };
    au_set(unit, AU_PROP_SET_RENDER_CALLBACK, AU_SCOPE_GLOBAL, 0, &cb, "SetRenderCallback")?;

    let rc = unsafe { AudioUnitInitialize(unit) };
    if rc != 0 {
        bail!("AudioUnitInitialize(speaker) failed (OSStatus {rc})");
    }
    Ok(unit)
}
...
pub fn run(cfg: LiveConfig) -> Result<()> {
    ...
    println!("vdev device : #{} \"{}\"  uid={}", vdev_info.id, vdev_info.name, vdev_info.uid);
    println!("              {:.0} Hz, buffer {} frames, {} ch out / {} ch in, reported latency {:.2} ms",
        vdev_info.sample_rate, vdev_info.buffer_frames,
        vdev_info.output_channels, vdev_info.input_channels, vdev_info.reported_latency_ms);
    ...
    if let Some(d) = speaker_dev {
        let i = describe(d);
        println!("speaker     : #{} \"{}\"  {:.0} Hz, {} ch out", i.id, i.name, i.sample_rate, i.output_channels);
    }
    ...
    if let Some(e) = &engine {
        println!("engine      : {}  ({} B state/stream)", e.path.display(), e.state_bytes);
    }

    let frame_ms = if let Some(e) = &engine { e.frame_size as f64 / 48.0 } else { 10.0 };
    ...
    // ---- injection side ---------------------------------------------------
    let mut inject = Box::new(InjectCtx {
        ...
        starved_before: 0,
        probe: is_probe,
        emit: (probe_mode == Some(ProbeMode::Digital)).then(|| MarkerEmitter::new(probe_period_frames)),
        marker: marker(),
        search: Vec::with_capacity(SEARCH_CAP + RENDER_SCRATCH),
        pending: Arc::clone(&pending),
        latency: LatencyProbe::new(),
        threshold: if probe_mode == Some(ProbeMode::Acoustic) { 0.35 } else { 0.60 },
    });
    let mut ioproc_id: AudioDeviceIOProcID = std::ptr::null_mut();
    let rc = unsafe {
        AudioDeviceCreateIOProcID(vdev, inject_ioproc, &mut *inject as *mut InjectCtx as *mut c_void, &mut ioproc_id)
    };
    if rc != 0 {
        bail!("AudioDeviceCreateIOProcID(vdev) failed (OSStatus {rc})");
    }
    ...
    let speaker_unit = match (probe_mode, speaker_dev) {
        (Some(ProbeMode::Acoustic), Some(d)) => {
            ...
            let ctx = Box::new(SpeakerCtx {
                ...
                pos: 0,
                until_next: 0,
                period_frames: probe_period_frames,
    ...
    if let Some((unit, _)) = &capture_unit {
        ...
        if rc != 0 {
            bail!("AudioOutputUnitStart(capture) failed (OSStatus {rc})");
        }
    }
    if let Some((unit, _)) = &speaker_unit {
        let rc = unsafe { AudioOutputUnitStart(*unit) };
        if rc != 0 {
            bail!("AudioOutputUnitStart(speaker) failed (OSStatus {rc})");
        }
    }
    let rc = unsafe { AudioDeviceStart(vdev, ioproc_id) };
    if rc != 0 {
        bail!("AudioDeviceStart(vdev) failed (OSStatus {rc})");
    }
    ...
    while wall0.elapsed() < target {
        ...
        if is_probe {
            ...
            println!("  {:.0}s  markers detected: {}", wall0.elapsed().as_secs_f64(), n);
        } else {
            let core = capture_unit.as_ref().map(|(_, p)| unsafe { &**p });
            if let Some(c) = core {
                println!("  {:.0}s  frames: {}", wall0.elapsed().as_secs_f64(), c.frames);
            }
        }
    }
    ...
    let audio_seconds = core.map(|c| c.frames as f64 * frame_ms / 1000.0).unwrap_or(0.0);
    let report = RunReport {
        ...
        audio_seconds,
        wall_seconds: wall,
        vad_mean: core.map(|c| c.vad_sum / c.frames.max(1) as f64).unwrap_or(0.0),
        wet_ratio_mean: core.map(|c| c.wet_sum / c.frames.max(1) as f64).unwrap_or(0.0),
        wet_ratio_min: core.and_then(|c| (c.wet_min != f64::MAX).then_some(c.wet_min)).unwrap_or(0.0),
        wet_ratio_max: core.and_then(|c| (c.wet_max != f64::MIN).then_some(c.wet_max)).unwrap_or(0.0),
        ...
        cpu_seconds: cpu,
        cpu_percent_of_one_core: if audio_seconds > 0.0 { cpu / audio_seconds * 100.0 } else { 0.0 },
        latency: if is_probe {
    ...
    if let Some(c) = core {
        if let Some(p) = &cfg.record_in {
            crate::wavio::write(p, &c.rec_in).with_context(|| format!("writing {}", p.display()))?;
            println!("captured    : {} ({:.1} s)", p.display(), c.rec_in.len() as f64 / 48_000.0);
        }
        if let Some(p) = &cfg.record_out {
            crate::wavio::write(p, &c.rec_out).with_context(|| format!("writing {}", p.display()))?;
            println!("injected    : {} ({:.1} s)", p.display(), c.rec_out.len() as f64 / 48_000.0);
        }
    }
...
fn print_report(r: &RunReport) {
    ...
    if let Some(l) = &r.latency {
        println!("round trips : {} detected of {} injected ({} rejected)",
            l.count, l.injected, l.rejected);
        println!("latency     : min {:.2}  mean {:.2}  p50 {:.2}  p95 {:.2}  max {:.2}  sd {:.2} ms",
            l.min_ms, l.mean_ms, l.p50_ms, l.p95_ms, l.max_ms, l.stdev_ms);
        println!("              {}", l.interpretation);
    }
    if r.frames > 0 {
        println!("frames      : {}  ({:.2} s of audio in {:.2} s wall)", r.frames, r.audio_seconds, r.wall_seconds);
        println!("model time  : p50 {:.4}  p95 {:.4}  p99 {:.4}  max {:.4} ms  (budget {:.1} ms)",
            r.frame_ms_p50, r.frame_ms_p95, r.frame_ms_p99, r.frame_ms_max, r.frame_ms);
        println!("cpu         : {:.2} s -> {:.2} % of one core", r.cpu_seconds, r.cpu_percent_of_one_core);
        println!("wet ratio   : mean {:.3}  min {:.3}  max {:.3}", r.wet_ratio_mean, r.wet_ratio_min, r.wet_ratio_max);
    }
    println!("ring        : dropped {} samples, starved {} samples",
