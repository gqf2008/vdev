...
- (CFTypeRef)clock {
    if (_clock == NULL) {
        OSStatus err = CMIOStreamClockCreate(kCFAllocatorDefault, CFSTR("VDCamera::VDStream::clock"), (__bridge void *)self,  CMTimeMake(1, 10), 100, 10, &_clock);
        if (err != noErr) {
...
- (CVPixelBufferRef)createPixelBufferWithTestAnimation {
    int width = VDCAMERA_WIDTH;
    int height = VDCAMERA_HEIGHT;

    CVPixelBufferRef pxbuffer = NULL;
    CVReturn status = CVPixelBufferCreate(kCFAllocatorDefault, width, height, kCVPixelFormatType_32ARGB, NULL, &pxbuffer);
    if (status != kCVReturnSuccess || pxbuffer == NULL) {
        DLog(@"CVPixelBufferCreate failed: %d", status);
        return NULL;
    }
    ...
    // 帧内容来自 Rust 核心（vdev-camera），图案 0 = SMPTE 彩条。
    double time = double(mach_absolute_time()) / NSEC_PER_SEC;
    int rc = vdev_camera_render_argb32(0, width, height, time,
                                       (unsigned char *)pxdata, bytesPerRow * height);
    if (rc != 0) {
        DLog(@"vdev_camera_render_argb32 failed: %d", rc);
    }
...
- (UInt32)getPropertyDataSizeWithAddress:(CMIOObjectPropertyAddress)address qualifierDataSize:(UInt32)qualifierDataSize qualifierData:(nonnull const void *)qualifierData {
    switch (address.mSelector) {
        ...
        default:
            DLog(@"VDStream unhandled getPropertyDataSizeWithAddress for %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
            return 0;
    };
}
...
- (void)getPropertyDataWithAddress:(CMIOObjectPropertyAddress)address qualifierDataSize:(UInt32)qualifierDataSize qualifierData:(nonnull const void *)qualifierData dataSize:(UInt32)dataSize dataUsed:(nonnull UInt32 *)dataUsed data:(nonnull void *)data {
    switch (address.mSelector) {
        case kCMIOObjectPropertyName:
            *static_cast<CFStringRef*>(data) = CFSTR("VDCamera VDStream");
            *dataUsed = sizeof(CFStringRef);
            break;
        case kCMIOObjectPropertyElementName:
            *static_cast<CFStringRef*>(data) = CFSTR("VDCamera VDStream Element");
            *dataUsed = sizeof(CFStringRef);
            break;
        case kCMIOObjectPropertyManufacturer:
        case kCMIOObjectPropertyElementCategoryName:
        ...
        case kCMIOStreamPropertyInitialPresentationTimeStampForLinkedAndSyncedAudio:
        case kCMIOStreamPropertyOutputBuffersNeededForThrottledPlayback:
            DLog(@"TODO: %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
            break;
        case kCMIOStreamPropertyDirection:
        ...
        case kCMIOStreamPropertyClock:
            ...
            *dataUsed = sizeof(CFTypeRef);
            break;
        default:
            DLog(@"VDStream unhandled getPropertyDataWithAddress for %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
            *dataUsed = 0;
    };
}
...
- (BOOL)hasPropertyWithAddress:(CMIOObjectPropertyAddress)address {
    switch (address.mSelector){
        ...
        case kCMIOStreamPropertyOutputBuffersNeededForThrottledPlayback:
            ...
            return false;
        default:
            DLog(@"VDStream unhandled hasPropertyWithAddress for %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
            return false;
    };
}

- (BOOL)isPropertySettableWithAddress:(CMIOObjectPropertyAddress)address {
    DLog(@"VDStream unhandled isPropertySettableWithAddress for %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
    return false;
}
...
- (void)setPropertyDataWithAddress:(CMIOObjectPropertyAddress)address qualifierDataSize:(UInt32)qualifierDataSize qualifierData:(nonnull const void *)qualifierData dataSize:(UInt32)dataSize data:(nonnull const void *)data {
    DLog(@"VDStream unhandled setPropertyDataWithAddress for %@", [VDObjectStore StringFromPropertySelector:address.mSelector]);
}
