...
fn main() -> Result<(), String> {
    ...
    //    不需要再自己跑 signal 保活线程。
    let (_sip, mut endpoint, mut socket, _video_mid, _audio_mid, _camera_mid) = connect_viewer_sip(
        server,
        room,
        auth,
    ...
    // 4. 解码器 + 组装器
    let mut decoder: Option<codec::VideoDecoder> = None;
    ...
    let mut opus_decoder = audio_sink.as_ref().and_then(|_| codec::OpusDecoder::new().ok());
        ...
        while let Some(ev) = endpoint.poll_event() {
            if let ClientEvent::Media(data) = ev {
                ...
                if decoder.as_ref().map(|d| d.codec() != codec).unwrap_or(true) {
                    decoder = codec::VideoDecoder::new(codec).ok();
                }
                ...
                let Ok(Some(frame)) = dec.decode(&au.data) else { continue };

                // RGBA → BGRA + 缩放（帧分辨率可能是源分辨率，需缩放到输出）
                let mut bgra =
                    rgba_to_bgra_scaled(&frame.rgba, frame.width, frame.height, out_w, out_h);
