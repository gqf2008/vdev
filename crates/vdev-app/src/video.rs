...
pub fn host_time_ns() -> u64 {
    // SAFETY：CMClockGetHostTimeClock 返回系统时钟单例，CMClockGetTime 只读
    let clock = unsafe { ffi::CMClockGetHostTimeClock() };
    let t = unsafe { ffi::CMClockGetTime(clock) };
...
pub fn ensure_nsapp() {
    ...
    // SAFETY：sharedApplication 为无参类方法，返回系统 NSApplication 单例
    let app: *mut objc2::runtime::AnyObject = unsafe { objc2::msg_send![cls, sharedApplication] };
    if !app.is_null() {
        // SAFETY：app 非空时指向 NSApplication 实例
        let app_ref: &objc2::runtime::AnyObject = unsafe { &*app };
        let _: () = unsafe { objc2::msg_send![app_ref, setActivationPolicy: 0] }; // Regular
        let _: bool = unsafe { objc2::msg_send![app_ref, activateIgnoringOtherApps: true] };
    }
}
...
pub fn pick_video() -> Result<Option<PickedVideo>, String> {
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(
        || -> Result<Option<PickedVideo>, String> {
            let mtm = objc2::MainThreadMarker::new()
                .ok_or_else(|| "文件选择器必须在主线程打开".to_string())?;
            #[allow(deprecated)]
            let panel = NSOpenPanel::openPanel(mtm); // 沙盒缺 user-selected 权限时返回 NULL → panic
                                                     // SAFETY：panel 非空（上一行 deprecated openPanel 在缺权限时才返回 NULL，见注释），
                                                     // 各 setter 为无副作用配置消息
            let _: () = unsafe { msg_send![&*panel, setCanChooseFiles: true] };
            let _: () = unsafe { msg_send![&*panel, setCanChooseDirectories: false] };
            ...
            #[allow(deprecated)]
            let _: () = unsafe { msg_send![&*panel, setAllowedFileTypes: Some(&*array)] };
...
fn filter_params_from_env() -> vdev_filter::FilterParams {
    ...
    if let Ok(v) = std::env::var("VDEV_FILTER") {
        ...
        if !parts.is_empty() {
            p.brightness = parts[0];
        }
        if parts.len() >= 2 {
            p.contrast = parts[1];
        }
        if parts.len() >= 3 {
            p.saturation = parts[2];
        }
        if parts.len() >= 4 {
            p.green_screen_threshold = parts[3].clamp(0.0, 255.0) as u8;
        }
        if parts.len() >= 5 {
            p.sharpen = parts[4];
        }
        if parts.len() >= 6 {
            p.beauty_strength = parts[5].clamp(0.0, 1.0);
        }
        if parts.len() >= 7 {
            p.whiten_strength = parts[6].clamp(0.0, 1.0);
        }
    }
    p
}

/// 帧回调共享句柄（解码线程写入 / 保活线程重发共用）
type FrameCallback =
    std::sync::Arc<std::sync::Mutex<Box<dyn FnMut(Vec<u8>, u32, u32, u32) + Send>>>;
/// 最近一帧缓存（保活线程重发用）
type LastFrame = std::sync::Arc<std::sync::Mutex<Option<(Vec<u8>, u32, u32, u32)>>>;

/// done 置位守卫：解码线程任何退出路径（正常/出错/panic unwind）都保证置位，
/// 否则保活线程永久重发冻结帧、UI 卡在「停止视频推流」。
...
// 当前恒为 Ok，但保留 Result 供调用方以 Err 分支处理（调用点已按可失败接口书写）
...
pub fn push_video(
    ...
    let cb: FrameCallback = Arc::new(Mutex::new(Box::new(on_frame)));
    ...
    std::thread::spawn(move || {
        ...
        let _access = access;
        // done 守卫：正常结束 / 出错 / panic unwind 的任何退出路径都置位 done。
        // 之前 done 只在 run() 正常返回后手动置位——解码线程一旦 panic，保活线程
        // 永久重发冻结帧、UI 卡在「停止视频推流」。依赖真实解码时序，无法单测。
        let done_guard = DoneGuard(done_d);
        ...
        let r = run(&local, width, height, fps, move |buf, w, h, stride| {
            ...
            *frame_d
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner) = Some((buf, w, h, stride));
            *sent_d
                .lock()
        ...
        // 先置 done 再回调 on_done（守卫 Drop 兜底 panic 路径，这里显式保持原顺序）
        drop(done_guard);
        on_done();
    });
...
#[allow(deprecated)]
#[allow(clippy::too_many_lines)] // PTS 推流时序单循环直读，拆散反而难对照解码状态机
fn run(
    path: &str,
    width: u32,
    height: u32,
    ...
    let url =
        NSURL::fileURLWithPath_isDirectory_relativeToURL(&NSString::from_str(path), false, None);
    // SAFETY：以下 AVFoundation 调用均按 API 契约传有效对象
    ...
    .map_err(|e| {
        let desc = e
            .as_deref()
            .map_or_else(|| "未知异常".to_string(), |ex| error_description(ex));
    ...
    loop {
        ...
        // SAFETY：output 为已添加到 reader 的有效输出对象
        let sample: *mut std::ffi::c_void = unsafe { msg_send![&*output, copyNextSampleBuffer] };
        if sample.is_null() {
            break;
        }
        let pb = unsafe { ffi::CMSampleBufferGetImageBuffer(sample) };
        if pb.is_null() {
            unsafe { ffi::CFRelease(sample.cast_const()) };
            continue;
        }
        let pts = unsafe { ffi::CMSampleBufferGetPresentationTimeStamp(sample) };
        let pts_ns = if pts.timescale > 0 {
            (pts.value as f64 / f64::from(pts.timescale) * 1e9) as i64
        } else {
            0
        };
        let base = *base_pts_ns.get_or_insert(pts_ns);
        let target_ns = start_ns.saturating_add((pts_ns - base).max(0) as u64);
        unsafe { ffi::CVPixelBufferLockBaseAddress(pb, 1) }; // read-only
        let src_w = unsafe { ffi::CVPixelBufferGetWidth(pb) };
        let src_h = unsafe { ffi::CVPixelBufferGetHeight(pb) };
        let src_stride = unsafe { ffi::CVPixelBufferGetBytesPerRow(pb) };
        let base = unsafe { ffi::CVPixelBufferGetBaseAddress(pb) };
        let raw = if base.is_null() {
            Vec::new()
        } else {
            // SAFETY：base 指向 lock 期间有效的 stride*src_h 字节像素缓冲
            unsafe { std::slice::from_raw_parts(base as *const u8, src_stride * src_h) }.to_vec()
        };
        unsafe { ffi::CVPixelBufferUnlockBaseAddress(pb, 1) };
